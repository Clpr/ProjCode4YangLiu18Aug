...
for idx = 1:length(fore_est_Y)
	...
	for idxT = 1:-1:1
		...
		Weights(chk,:) = Weights(chk,:) .^( repmat(est_W',K,1) - 1 );
...
% parameter used: est_X
    ...
    est_Theta = X_est(5:4+L); 
    est_W = X_est((5+L):end);
    % 2. get weights （wrote a sub-function to keep compat)
    Weights = GetWeights_UnrestrictBeta( est_W,T,K,L );
