function LogL = objfunc(X,L,I,T,K,Y,Z)
%{
objective function of GARCH-MIDAS (multivariate, un-restricted 1 par Beta weighting)

Input:
1. X [col-vec,len=2+4]: in order: [Mu,Alpha,Beta,M,Theta1,W1]
...
% Cap1: Weighting Strategy (1 par Un-restricted Beta weighting strategy)
...
% Cap2: Estimate \tau_{t} (size=T*1) (long-term volatility)
	...
	for idxT = T:-1:1
		...
		Tau(idxT) = exp( M + sum(sum(Weights(chk_w,:).*Z(chk_z,:),1))* Theta );
	end
